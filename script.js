$(document).ready(function() {
    ...
    function renderCart() {
      ...
      cart.forEach((item, index) => {
        if (item.quantity > 0) { // Verifica que la cantidad sea mayor que 0
    ...
    function addToCart(product) {
      if (!product.name || isNaN(product.quantity)) { // Validar que los datos sean obligatorios y números
        alert('Todos los campos son obligatorios y deben ser números.');
        return;
      }
      
      if (product.quantity <= 0 || product.quantity > 10) { // Validar que la cantidad esté entre 1 y 10
        alert('La cantidad debe ser entre 1 y 10.');
        return;
      }
      
      const existingProductIndex = cart.findIndex(item => item.name === product.name);
      if (existingProductIndex !== -1) { // Si el producto ya existe en el carrito, suma la cantidad
        cart[existingProductIndex].quantity += product.quantity;
        if (cart[existingProductIndex].quantity > 10) cart[existingProductIndex].quantity = 10; // No puede ser mayor a 10
      } else {
        cart.push(product);
      }
      renderCart();
    ...
    function editItem(index) {
      ...
      $(`#item-${index}`).find('.btn-info').hide(); // Esconde el botón de editar
      $(`#item-${index}`).append(saveButton); // Agrega el botón de guardar
    }
    ...
    // Load predefined products into cart
